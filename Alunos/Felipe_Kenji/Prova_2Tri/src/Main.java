// Compilar javac -cp "lib/*" src/*.java -d out
// Rodar java -cp "out;lib/*" Main
        ...
        do {
            System.out.println("\n--- Minha Lista de " + listName + " (" + list.size() + " séries) -
                ...
                Dados.Serie serie = list.get(i);
                        rie.getRating() != null && serie.getRating().getAverage() != null) ?
                               String.format("%.1f", serie.getRating().getAverage()) : "N/A";
                System.out.printf("%d. [ID: %d] %s (Status: %s, Nota: %
