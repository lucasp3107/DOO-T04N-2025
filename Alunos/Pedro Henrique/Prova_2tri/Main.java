...
import com.minhas_series_tv.util.JsonDataManager;
        ...
        do {
            try {
                ...
                switch (opcao) {
                    ...
                    case 2:
                        List<Serie> favoritos = serieManager.getListaFavoritos();
                        gerenciarLista(favoritos, "Séries Favoritas", serieManager, "favoritos", menuView);
                        break;
                    case 3:
                        List<Serie> assistidas = serieManager.getListaAssistidas();
                        gerenciarLista(assistidas, "Séries Já Assistidas", serieManager, "assistidas", menuView);
                        break;
                    case 4:
                        List<Serie> desejoAssistir = serieManager.getListaDesejoAssistir();
                        gerenciarLista(desejoAssistir, "Séries Que Desejo Assistir", serieManager, "desejoAssistir", menuView);
                        break;
                    case 5:
    ...
    private static void gerenciarLista(List<Serie> lista, String titulo, SerieManager serieManager, String tipoLista, MenuView menuView) {
        ...
        do {
            ...
            if (escolhaRemocao > 0 && escolhaRemocao <= lista.size()) {
                ...
                switch (tipoLista) {
                    case "favoritos":
                        removido = serieManager.removerSerieFavoritos(serieParaRemover.getId());
                        break;
                    case "assistidas":
                        removido = serieManager.removerSerieAssistida(serieParaRemover.getId());
                        break;
                    case "desejoAssistir":
                        removido = serieManager.removerSerieDesejaAssistir(serieParaRemover.getId());
                        break;
                }
                if (removido) {
